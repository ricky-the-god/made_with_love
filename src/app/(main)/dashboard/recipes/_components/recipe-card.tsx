...
export function RecipeCard({ recipe, href }: RecipeCardProps) {
  ...
  const ratings = Array.isArray(recipe.recipe_ratings) ? recipe.recipe_ratings : [];
  const ratingCount = ratings.length;
  const averageRating = ratingCount > 0 ? ratings.reduce((sum, review) => sum + review.rating, 0) / ratingCount : null;
